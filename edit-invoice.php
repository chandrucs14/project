...
try {
    $stmt = $pdo->prepare("
        SELECT i.*, c.name as customer_name, c.customer_code, c.phone, c.email, c.gst_number, 
               c.address, c.city, c.state, c.pincode, c.outstanding_balance, c.credit_limit
        FROM invoices i
    ...
    $itemsStmt = $pdo->prepare("
        SELECT ii.*, p.name as product_name, p.unit, g.gst_rate
...
$invoice_data = [
    ...
    'customer_outstanding' => $invoice['outstanding_balance'],
    'customer_credit_limit' => $invoice['credit_limit'],
    'payment_type' => $invoice['payment_type'],
    'status' => $invoice['status'],
    'notes' => $invoice['notes'],
    'terms' => $invoice['terms'] ?? $settings['invoice_footer_text'] ?? '',
    'subtotal' => floatval($invoice['subtotal']),
    'gst_total' => floatval($invoice['gst_total']),
    'discount_amount' => floatval($invoice['discount_amount']),
    'discount_type' => 'fixed', // Will calculate based on values
    'discount_value' => floatval($invoice['discount_amount']),
    'total_amount' => floatval($invoice['total_amount']),
    'items' => $items
];

// Calculate discount type and value
$subtotal_with_gst = $invoice_data['subtotal'] + $invoice_data['gst_total'];
if ($subtotal_with_gst > 0) {
    $discount_percentage = ($invoice_data['discount_amount'] / $subtotal_with_gst) * 100;
    // If discount percentage is a nice round number, use percentage type
    if (abs($discount_percentage - round($discount_percentage)) < 0.01) {
        $invoice_data['discount_type'] = 'percentage';
        $invoice_data['discount_value'] = round($discount_percentage);
    } else {
        $invoice_data['discount_type'] = 'fixed';
        $invoice_data['discount_value'] = $invoice_data['discount_amount'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_invoice') {
    ...
    $invoice_data['customer_id'] = intval($_POST['customer_id'] ?? $invoice_data['customer_id']);
    $invoice_data['payment_type'] = $_POST['payment_type'] ?? $invoice_data['payment_type'];
    $invoice_data['status'] = $_POST['status'] ?? $invoice_data['status'];
    $invoice_data['notes'] = $_POST['notes'] ?? '';
    $invoice_data['terms'] = $_POST['terms'] ?? $settings['invoice_footer_text'] ?? '';
    $invoice_data['discount_type'] = $_POST['discount_type'] ?? 'fixed';
    $invoice_data['discount_value'] = floatval($_POST['discount_value'] ?? 0);
    ...
    $invoice_data['gst_total'] = $gst_total;
    
    // Calculate discount
    if ($invoice_data['discount_type'] === 'percentage') {
        $invoice_data['discount_amount'] = ($subtotal + $gst_total) * ($invoice_data['discount_value'] / 100);
    } else {
        $invoice_data['discount_amount'] = $invoice_data['discount_value'];
    }
    
    $invoice_data['total_amount'] = $subtotal + $gst_total - $invoice_data['discount_amount'];
    $invoice_data['outstanding_amount'] = ($invoice_data['status'] === 'paid') ? 0 : $invoice_data['total_amount'];
    $invoice_data['paid_amount'] = ($invoice_data['status'] === 'paid') ? $invoice_data['total_amount'] : 0;
    ...
    if (empty($errors)) {
        try {
            ...
            foreach ($items as $item) {
                $original_item = array_filter($invoice_data['items'], function($i) use ($item) {
                    return $i['product_id'] == $item['product_id'];
                });
                
                $original_quantity = !empty($original_item) ? floatval(reset($original_item)['quantity']) : 0;
            ...
            $deleteItemsStmt->execute([$invoice_id]);
            ...
            $updateCustomerStmt->execute([
                ':amount' => $invoice_data['outstanding_amount'] - $invoice['outstanding_amount'],
                ':customer_id' => $invoice_data['customer_id']
            ]);
            ...
            $logStmt->execute([
                ...
                ':description' => "Invoice updated: " . $invoice_data['invoice_number'],
            ...
            header("Location: view-invoice.php?id=" . $invoice_id . "&success=1");
            exit();
            
        } catch (Exception $e) {
            $pdo->rollBack();
            $errors['database'] = "Failed to update invoice: " . $e->getMessage();
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <h4 class="mb-0 font-size-18">Edit Invoice #<?= htmlspecialchars($invoice['invoice_number']) ?></h4>
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                                    <li class="breadcrumb-item"><a href="invoices.php">Invoices</a></li>
                                    <li class="breadcrumb-item"><a href="view-invoice.php?id=<?= $invoice_id ?>">View Invoice</a></li>
                ...
                <!-- Invoice Form -->
                <form method="POST" action="edit-invoice.php?id=<?= $invoice_id ?>" id="invoiceForm">
                        ...
                        <div class="col-xl-8">
                            <div class="card">
                                <div class="card-body">
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                <input type="date" class="form-control <?= isset($errors['invoice_date']) ? 'is-invalid' : '' ?>" 
                                                       id="invoice_date" name="invoice_date" value="<?= htmlspecialchars($invoice_data['invoice_date']) ?>" required>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                <input type="date" class="form-control <?= isset($errors['due_date']) ? 'is-invalid' : '' ?>" 
                                                       id="due_date" name="due_date" value="<?= htmlspecialchars($invoice_data['due_date']) ?>" required>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-control select2 <?= isset($errors['customer']) ? 'is-invalid' : '' ?>" 
                                                    ...
                                                    <option value="<?= $customer['id'] ?>" 
                                                            <?= $customer['id'] == $invoice_data['customer_id'] ? 'selected' : '' ?>
                                        ...
                                        <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-control" id="payment_type" name="payment_type">
                                                    <option value="credit" <?= $invoice_data['payment_type'] == 'credit' ? 'selected' : '' ?>>Credit</option>
                                                    <option value="cash" <?= $invoice_data['payment_type'] == 'cash' ? 'selected' : '' ?>>Cash</option>
                                                    <option value="bank_transfer" <?= $invoice_data['payment_type'] == 'bank_transfer' ? 'selected' : '' ?>>Bank Transfer</option>
                                                    <option value="cheque" <?= $invoice_data['payment_type'] == 'cheque' ? 'selected' : '' ?>>Cheque</option>
                                                    <option value="online" <?= $invoice_data['payment_type'] == 'online' ? 'selected' : '' ?>>Online Payment</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Customer Info Card -->
                                    <div id="customerInfoCard" class="card bg-light mt-2" style="display: <?= $invoice_data['customer_id'] ? 'block' : 'none' ?>;">
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <p class="mb-1"><i class="bi bi-building me-1"></i> <strong>GST Number:</strong> <span id="customerGst"><?= htmlspecialchars($invoice_data['customer_gst'] ?? '-') ?></span></p>
                                                    <p class="mb-1"><i class="bi bi-cash-stack me-1"></i> <strong>Outstanding:</strong> <span id="customerOutstanding">₹<?= number_format($invoice_data['customer_outstanding'] ?? 0, 2) ?></span></p>
                                                </div>
                                                <div class="col-md-6">
                                                    <p class="mb-1"><i class="bi bi-credit-card me-1"></i> <strong>Credit Limit:</strong> <span id="customerCreditLimit">₹<?= number_format($invoice_data['customer_credit_limit'] ?? 0, 2) ?></span></p>
                                                    <p class="mb-1"><i class="bi bi-piggy-bank me-1"></i> <strong>Available Credit:</strong> <span id="customerAvailableCredit">₹<?= number_format(($invoice_data['customer_credit_limit'] ?? 0) - ($invoice_data['customer_outstanding'] ?? 0), 2) ?></span></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <input type="hidden" name="items" id="itemsInput" value='<?= json_encode($invoice_data['items']) ?>'>
                                </div>
                            </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <textarea class="form-control" id="notes" name="notes" rows="3" 
                                                          placeholder="Additional notes for customer..."><?= htmlspecialchars($invoice_data['notes']) ?></textarea>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                <label for="terms" class="form-label">Terms & Conditions</label>
                                                <textarea class="form-control" id="terms" name="terms" rows="3"><?= htmlspecialchars($invoice_data['terms']) ?></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card-body">
                                        ...
                                        <div class="row g-2">
                                            <div class="col-5">
                                                <select class="form-control" id="discount_type" name="discount_type">
                                                    <option value="percentage" <?= $invoice_data['discount_type'] == 'percentage' ? 'selected' : '' ?>>Percentage (%)</option>
                                                    <option value="fixed" <?= $invoice_data['discount_type'] == 'fixed' ? 'selected' : '' ?>>Fixed (₹)</option>
                                                </select>
                                            </div>
                                            <div class="col-7">
                                                <input type="number" class="form-control" id="discount_value" name="discount_value" 
                                                       value="<?= $invoice_data['discount_value'] ?>" min="0" step="0.01" placeholder="0.00">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-borderless mb-0">
                                            <tr>
                                                <td>Subtotal:</td>
                                                <td class="text-end" id="summarySubtotal">₹<?= number_format($invoice_data['subtotal'], 2) ?></td>
                                            </tr>
                                            <tr>
                                                <td>GST Total:</td>
                                                <td class="text-end" id="summaryGst">₹<?= number_format($invoice_data['gst_total'], 2) ?></td>
                                            </tr>
                                            <tr>
                                                <td>Discount:</td>
                                                <td class="text-end" id="summaryDiscount">-₹<?= number_format($invoice_data['discount_amount'], 2) ?></td>
                                            </tr>
                                            <tr class="border-top">
                                                <th>Total Amount:</th>
                                                <th class="text-end" id="summaryTotal">₹<?= number_format($invoice_data['total_amount'], 2) ?></th>
                                            </tr>
                                        </table>
                                    </div>
                                    ...
                                    <div class="d-grid gap-2 mt-4">
                                        ...
                                        <a href="view-invoice.php?id=<?= $invoice_id ?>" class="btn btn-outline-secondary">
                                            <i class="mdi mdi-eye"></i> View Invoice
                                        </a>
                                    ...
                                    <!-- Warning Message -->
                                    <div class="mt-4 p-3 bg-warning bg-soft rounded">
                                        <h6 class="mb-2"><i class="mdi mdi-alert-circle text-warning me-1"></i> Important Note</h6>
                                        <p class="small text-muted mb-0">
                                            Editing this invoice will update stock quantities and customer outstanding balance. 
...
<script>
    ...
    $('#invoiceForm').submit(function(e) {
        ...
        return confirm('Are you sure you want to update this invoice? This will affect stock levels and customer balance.');
    });

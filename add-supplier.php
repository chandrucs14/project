...
function generateSupplierCode($pdo) {
    try {
        ...
        $stmt = $pdo->query("SELECT supplier_code FROM suppliers WHERE supplier_code LIKE 'SUP%' ORDER BY id DESC LIMIT 1");
        ...
        // If there's an error, generate a code based on timestamp
        return 'SUP' . date('ymd') . rand(100, 999);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_supplier'])) {
    ...
    $force_create = isset($_POST['force_create']) ? true : false;
    
    // Debug: Log the received data
    error_log("Received supplier data: " . print_r($_POST, true));
    ...
    if (empty($name)) {
        ...
        try {
            // Check if phone number already exists (unless force_create is true)
            if (!$force_create) {
                $checkStmt = $pdo->prepare("SELECT id, supplier_code, name, company_name, email, phone FROM suppliers WHERE phone = ?");
                $checkStmt->execute([$phone]);
                $existing = $checkStmt->fetch();
                
                if ($existing) {
                    $existing_supplier = $existing;
                    throw new Exception("A supplier with this phone number already exists.");
                }
            }
            
            // Check if email already exists (if provided)
            if (!empty($email)) {
                $checkEmailStmt = $pdo->prepare("SELECT id FROM suppliers WHERE email = ? AND phone != ?");
                $checkEmailStmt->execute([$email, $phone]);
                if ($checkEmailStmt->fetch()) {
                    throw new Exception("A supplier with this email address already exists.");
                }
            }
            
            // Check if GST number already exists (if provided)
            if (!empty($gst_number)) {
                $checkGstStmt = $pdo->prepare("SELECT id FROM suppliers WHERE gst_number = ? AND phone != ?");
                $checkGstStmt->execute([$gst_number, $phone]);
                if ($checkGstStmt->fetch()) {
                    throw new Exception("A supplier with this GST number already exists.");
                }
            }
            
            // Start transaction only after all checks are passed
            ...
            $stmt = $pdo->prepare("
                INSERT INTO suppliers (
                    supplier_code, name, company_name, email, phone, alternate_phone,
                    address, city, state, pincode, gst_number, pan_number,
                    payment_terms, outstanding_balance, is_active,
                    created_at, created_by
                ) VALUES (
                    :supplier_code, :name, :company_name, :email, :phone, :alternate_phone,
                    :address, :city, :state, :pincode, :gst_number, :pan_number,
                    :payment_terms, 0.00, :is_active,
                    NOW(), :created_by
                )
            ");
            ...
            if ($result) {
                ...
                $activity_stmt->execute([
                    $_SESSION['user_id'],
                    'New supplier created: ' . $name,
                    $activity_data
                ]);
                
                $pdo->commit();
                
                $_SESSION['success_message'] = "Supplier created successfully. Supplier Code: " . $supplier_code;
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <h4 class="mb-0 font-size-18">Add New Supplier</h4>
                ...
                <!-- Existing Supplier Warning -->
                <?php if ($existing_supplier): ?>
                <div class="row">
                    <div class="col-12">
                        <div class="alert alert-warning alert-dismissible fade show" role="alert">
                            <i class="mdi mdi-alert-outline me-2"></i>
                            <strong>Duplicate Found!</strong> A supplier with this phone number already exists.
                            <hr>
                            <div class="row">
                                <div class="col-md-3">
                                    <strong>Code:</strong> <?= htmlspecialchars($existing_supplier['supplier_code']) ?>
                                </div>
                                <div class="col-md-3">
                                    <strong>Name:</strong> <?= htmlspecialchars($existing_supplier['name']) ?>
                                </div>
                                <div class="col-md-3">
                                    <strong>Company:</strong> <?= htmlspecialchars($existing_supplier['company_name'] ?? 'N/A') ?>
                                </div>
                                <div class="col-md-3">
                                    <strong>Phone:</strong> <?= htmlspecialchars($existing_supplier['phone']) ?>
                                </div>
                            </div>
                            <div class="mt-2">
                                <a href="view-supplier.php?id=<?= $existing_supplier['id'] ?>" class="btn btn-sm btn-primary">
                                    <i class="mdi mdi-eye me-1"></i> View Existing Supplier
                                </a>
                                <button type="button" class="btn btn-sm btn-success" onclick="continueWithDuplicate()">
                                    <i class="mdi mdi-plus me-1"></i> Create Anyway
                                </button>
                                <button type="button" class="btn btn-sm btn-secondary" onclick="window.location.href='add-supplier.php'">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="card-title-desc">Enter the details of the new supplier</p>
                                ...
                                <form method="POST" action="" id="supplierForm">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" 
                                                           ...
                                                           value="<?= safe_echo($_POST['company_name'] ?? '') ?>">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="tel" 
                                                           ...
                                                           maxlength="10"
                                                           pattern="[0-9]{10}"
                                                           value="<?= safe_echo($_POST['alternate_phone'] ?? '') ?>">
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="number" 
                                                           ...
                                                           min="0"
                                                           max="365"
                                                           value="<?= safe_echo($_POST['payment_terms'] ?? '30') ?>">
                                                </div>
                                                <small class="text-muted">Number of days for payment (default: 30)</small>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea name="address" 
                                                  class="form-control" 
                                                  rows="3"
                                                  placeholder="Street address, building, area"><?= safe_echo($_POST['address'] ?? '') ?></textarea>
                                    </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" 
                                                           ...
                                                           maxlength="15"
                                                           value="<?= safe_echo($_POST['gst_number'] ?? '') ?>">
                                                </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" 
                                                           ...
                                                           maxlength="10"
                                                           value="<?= safe_echo($_POST['pan_number'] ?? '') ?>">
                                                </div>
                                    ...
                                    <div class="alert alert-info">
                                        ...
                                        A unique supplier code will be automatically generated (Format: SUPYYMMXXXX)
                                    </div>

                                    <div class="row mt-4">
                                        <div class="col-12">
                                            <div class="text-end">
                                                <a href="manage-suppliers.php" class="btn btn-secondary me-2">
                                                    <i class="mdi mdi-arrow-left me-1"></i> Cancel
                                                </a>
                                                <button type="button" class="btn btn-warning me-2" id="resetBtn">
                                                    <i class="mdi mdi-undo me-1"></i> Reset
                                                </button>
                                                <button type="submit" name="add_supplier" class="btn btn-success" id="submitBtn">
...
<script>
    ...
    document.getElementById('supplierForm')?.addEventListener('submit', function(e) {
        ...
        // Don't disable button immediately, let the form submit
        setTimeout(function() {
            btn.disabled = true;
    ...
    // Continue with duplicate creation
